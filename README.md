...
- **展示**：只要 `followUpRemark` 有值，详情页即展示，不受标讯状态限制（已转化 / 已放弃状态下亦可查看历史跟进记录）
- **编辑**：仅当标讯状态为 `following`（跟进中）时显示卡片右上角「编辑」按钮；其他状态下为只读展示
...
- **事业部**：默认取自标讯 BU，只读不可编辑
- **是否KT商机**（新增）：是/否，无默认值；选「是」时锁定产品域为「标准产品」
- **商机来源**：默认「标讯转化」，只读不可编辑
- **商机名称**：自动取自标讯项目名称
- **客户名称**：自动填充客户名称（取自标讯采购单位），只读不可编辑
- **CDBID**：必填，有值时只读展示，无值时内联搜索框支持按 CDBID/客户名称模糊搜索选择
- **商机阶段**：系统锁定「发现需求」，不可编辑
- **采购模式**：[普通采购、统签分采（总部）]，默认「普通采购」
- **产品域**：与「是否KT商机」联动 — 选「是」锁定「标准产品」；选「否」可在 [标准产品、简单方案、KT] 之间切换
- **预计签约日期**：仅允许选择当前日及未来日期（前端 min 限制 + 提示文案）
- **预计收入总金额（元）**（新增）：非必填数字输入
- **总台数**（新增）：非必填数字输入
- **赢率**：默认「10%(项目筹备期)」
- **是否有解决方案机会**：是/否
- **备注**：默认填充标讯摘要 `summary`，仍可手动编辑补充

> 说明：原「产品明细」模块（含方案/立项产品组、物料产品组、产线、按组拆分的预计收入金额）已废弃，由表单层级的「预计收入总金额（元）」「总台数」两字段统一替代。
...
**事件语义约束**（用于看板正确解读）：
- `编辑跟进备注 / 商机处理`：仅在标讯状态为 `following`（跟进中）时才会暴露入口，事件 100% 来自跟进中状态，可作为「跟进中状态有效编辑」漏斗节点统计
- `提交「跟进中」反馈 / 商机处理`：用户首次填写跟进备注并将状态置为 following 的入口事件
- `填写预计收入总金额 / 填写总台数 / 商机处理`：新建商机表单非必填字段，事件量反映用户对收入预估字段的实际填写率
- 商机来源、客户名称在新建商机表单中为只读字段，无对应埋点事件（只读字段不参与点击行为统计）
